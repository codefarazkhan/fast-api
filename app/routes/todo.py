...
@router.post("/")
def create_todo(todo: TodoCreate = Body(...), db=Depends(get_db_session), current_user=Depends(get_current_user)):
...
@router.put("/{todo_id}")
def update_todo(todo_id, todo_update: TodoUpdate = Body(...), db=Depends(get_db_session), current_user=Depends(get_current_user)):
    todo = db.query(Todo).filter(Todo.id == todo_id, Todo.user_id == current_user.id).first()
    if todo is None:
        raise HTTPException(status_code=404, detail="Todo not found")
